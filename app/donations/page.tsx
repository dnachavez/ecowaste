                ...
                try {
                    ...
                    if (snapshot && snapshot.materials) {
                        ...
                        if (request.materialId && materials[request.materialId]) {
                            ...
                            console.log('[MATERIAL_UPDATE] Found material via ID match:', targetMaterialId);
                        }
                        ...
                        if (!targetMaterialId) {
                            for (const matId in materials) {
                                ...
                                console.log('[MATERIAL_UPDATE] Checking material match:', { matId, matName, reqTitle });
                                if (matName.includes(reqTitle) || reqTitle.includes(matName)) {
                                    targetMaterialId = matId;
                                    console.log('[MATERIAL_UPDATE] Found material via Name match:', targetMaterialId);
                                    break;
                                }
                            }
                        }
                        ...
                        if (targetMaterialId) {
                            console.log('[MATERIAL_UPDATE] Update target determined:', targetMaterialId);
                            ...
                            console.log('[MATERIAL_UPDATE] Transaction details:', { path: `projects/${request.projectId}/materials/${targetMaterialId}/acquired`, adding: added });
                            try {
                                ...
                                console.log('[MATERIAL_UPDATE] Transaction completed successfully for materialId:', targetMaterialId, { committed: materialTxResult?.committed, newVal: materialTxResult?.snapshot?.val() });
                            } catch (txErr) {
                            ...
                            console.warn('[MATERIAL_UPDATE] No matching material found for request:', request.donationTitle);
                        }
                    }
                } catch (err) {
